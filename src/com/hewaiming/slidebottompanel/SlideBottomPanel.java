package com.hewaiming.slidebottompanel;

import com.hewaiming.slidebottomview.demo.R;
...
public class SlideBottomPanel extends FrameLayout {
	...
	public SlideBottomPanel(Context context, AttributeSet attrs, int defStyleAttr) {
		...
		MAX_CLICK_DISTANCE = mTitleHeightNoDisplay = a.getDimension(
				R.styleable.SlideBottomPanel_sbp_title_height_no_display, dp2px(DEFAULT_TITLE_HEIGHT_NO_DISPLAY));
	...
	private void handleActionUp(MotionEvent event) {
		...
		if (!isPanelShowing
			...
			displayPanel();
		} else if (!isPanelShowing && isDragging
				&& ((event.getY() - firstDownY > 0) || Math.abs(event.getY() - firstDownY) < mMoveDistanceToTrigger)) {
			hidePanel();
		}
	...
	private void handleActionMove(MotionEvent event) {
		...
		if (isDragging) {
			...
			if (mDarkFrameLayout != null && mIsFade) {
				...
				if (currentY > mMeasureHeight - mPanelHeight && currentY < mMeasureHeight - mTitleHeightNoDisplay) {
	...
	private void hidePanel() {
		...
		ValueAnimator animator = ValueAnimator.ofFloat(ViewHelper.getY(mPanel), mMeasureHeight - mTitleHeightNoDisplay);
	...
	private View findTopChildUnder(ViewGroup parentView, float x, float y) {
		...
		for (int i = childCount - 1; i >= 0; i--) {
			...
			if (x >= child.getLeft() && x < child.getRight() && y >= child.getTop() + mMeasureHeight - mPanelHeight
